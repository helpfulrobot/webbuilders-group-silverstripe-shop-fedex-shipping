...
class FedExShippingHelper {
    ...
    public static $service_types = array(
    	...
    	'GROUND_HOME_DELIVERY'		=> 'FedEx Ground Home',
	...
	public static function getServiceType($key){
		return isset(self::$service_types[$key]) ? self::$service_types[$key] : false;
	}
    ...
    {
        $this->wsdls_root = SHOP_FEDEX_SHIPPING_PATH.'/code/thirdparty/fedex';
        $this->defaults();
    }
    ...
    */
     ...
     *
    */
    public function addPackageLineItem($weight = array(), $dimensions = array(), $package_count = 1)
    {
        ...
        $sequence_number = count($this->request['RequestedShipment']['RequestedPackageLineItems']) + 1;
		...
		//Debug::dump($this->request['RequestedShipment']['RequestedPackageLineItems']);

/*	Not sure that this is necessary... because anytime you had more than 1 item it throws an error...
...
*/
     ...
     */
    public function getRates($dropOffType = 'REGULAR_PICKUP', $serviceType = false, $packagingType = 'YOUR_PACKAGING', $rateRequestType = 'LIST')
    {
        $this->validate(array('shipper','recipient'));
        $this->setTransactionDetail(__FUNCTION__);
        $this->setVersion('crs','14');
        ...
        if($serviceType){
 	       $shipmentSpecs['ServiceType'] = $serviceType;
        }
        
        $this->request['RequestedShipment'] = array_merge($this->request['RequestedShipment'], $shipmentSpecs);
     ...
     *
     */
    public function processShipment($dropOffType = 'REGULAR_PICKUP', $serviceType = 'FEDEX_GROUND', $packagingType = 'FEDEX_BOX', $rateRequestType = 'LIST')
    {
        $this->validate(array('shipper','recipient'));
        $this->setTransactionDetail(__FUNCTION__);
        $this->setVersion('ship','10');
    ...
    private function process($service, $method)
    {
        ...
        if($this->trace)
        {
            echo ' Service : '; self::pp($service);
            echo ' Method : '; self::pp($method);
            echo ' Request : '; self::pp($this->request);
        }
    ...
    {
        self::pp('<h2>Error returned in processing transaction</h2>');
        self::pp($response);
        die();
    }
